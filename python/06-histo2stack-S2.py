...
13.1.2023 modified for vegcover2023 -> farm -> parcel (affects only texts)

Make histo-files into annual dataframes. Saves into outputdir_annuals.

createMissingFiles() checks if all 10 bands exists per observation (parcel). If not, makes a copy of any band from the same doy and sets all values to zero.
...
python 06-histo2stack-S2.py -i  -n 32 -o  -t TEMPDIRPATH
...
def testing(all_files_df, out_dir_path, outputfile):
    ...
    if tmp2[tmp2.band != 10]['band'].any():
        ...
        print("All parcels have full 10 bands!")
    ...
    print("How many observations per parcel in one season (min)?: ", float(all_files_df[['parcelID', 'doy']].drop_duplicates().groupby(['parcelID']).count().min()))
    print("How many observations per parcel in one season (max)?: ", float(all_files_df[['parcelID', 'doy']].drop_duplicates().groupby(['parcelID']).count().max()))
