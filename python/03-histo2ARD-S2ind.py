...
2023-02-08 S2ind
2023-09-25 added new year 2023

From histograms to ARD data.

First:
# make ARD per date, all years by default:
python 03-histo2ARD-S2ind.py -i /Users/myliheik/Documents/myVEGCOVER/vegcover2023/S2ind/results-insitu -m -l l1

# Then this or straigh away this, combine intra-annual time series: (not done in vegcover2023)
python 03-histo2ARD-S2ind.py -i /Users/myliheik/Documents/myVEGCOVER/vegcover2023/S2ind/results-insitu -y 2022 -n -l l1

# Finally this combines years, give exact dates: (singleObservations)
python 03-histo2ARD-S2ind.py -i /Users/myliheik/Documents/myVEGCOVER/vegcover2023/S2ind/results-insitu -d 20200401 20210401 20220401 -s -l l1

# if non-normalized histogram data needed (e.g. for plotting):
python 03-histo2ARD-S2ind.py -i /Users/myliheik/Documents/myVEGCOVER/vegcover2023/S2indices/results-insitu -m
...
def make2D(ylist, results_folder_fp, norma):
    ...
    for file in inputfiles:
        ...
        if mission == 'S2':
            ...
            # toinen tapa, samat tulokset:
#            dfndbi = df[df['feature'] == 'ndbi']
#            dfndmi = df[df['feature'] == 'ndmi']
#            dfndti = df[df['feature'] == 'ndti']
#            dfndvi = df[df['feature'] == 'ndvi']

#            print(f"Lengths of subsets ndmi, ndti, ndvi: {len(dfndbi), len(dfndmi), len(dfndti), len(dfndvi)}")

#            dfndbipivot = dfndbi.pivot(index='parcelID', columns='date', values=bin32)
#            dfndbipivot.columns = ['_ndbi_'.join(col).strip() for col in dfndbipivot.columns.values]

#            dfndmipivot = dfndmi.pivot(index='parcelID', columns='date', values=bin32)
#            dfndmipivot.columns = ['_ndmi_'.join(col).strip() for col in dfndmipivot.columns.values]

#            dfndtipivot = dfndti.pivot(index='parcelID', columns='date', values=bin32)
#            dfndtipivot.columns = ['_ndti_'.join(col).strip() for col in dfndtipivot.columns.values]

#            dfndvipivot = dfndvi.pivot(index='parcelID', columns='date', values=bin32)
#            dfndvipivot.columns = ['_ndvi_'.join(col).strip() for col in dfndvipivot.columns.values]

#            all_dfs = pd.concat(
#                (iDF for iDF in [dfndbipivot, dfndmipivot, dfndtipivot, dfndvipivot]),
#                axis=1, join='inner'
#            ).reset_index()
            print(f"Shape of dataframe before munching: {df.shape}")
            print(f"Shape of pivoted dataframe: {pivoted.shape}")
...
def combineAnnual2D(years, inputdir, outputdir, norma):
    ...
    if '2018' in years:
        ...
        inputfiles = glob.glob(inputdir + '/2018*/ard2018S2.pkl')
        ...
        inputfiles = glob.glob(inputdir + '/2018*/meta2018S2.pkl')
    ...
    if '2019' in years:
        ...
        inputfiles = glob.glob(inputdir + '/2019*/ard2019S2.pkl')
        timeseries = pd.concat((iDF.set_index('parcelID') for iDF in map(pd.read_pickle, inputfiles)), sort=False, join='outer', axis=1).reset_index()

        inputfiles = glob.glob(inputdir + '/2019*/meta2019S2.pkl')
        metaseries = pd.concat(map(pd.read_pickle, inputfiles), sort=False, axis=0)
        metaseries2 = metaseries[chosenColumns].drop_duplicates()
    ...
    if '2020' in years:
        ...
        inputfiles = glob.glob(inputdir + '/2020*/ard2020S2.pkl')
        timeseries = pd.concat((iDF.set_index('parcelID') for iDF in map(pd.read_pickle, inputfiles)), sort=False, join='outer', axis=1).reset_index()

        inputfiles = glob.glob(inputdir + '/2020*/meta2020S2.pkl')
        metaseries = pd.concat(map(pd.read_pickle, inputfiles), sort=False, axis=0)
        metaseries2 = metaseries[chosenColumns].drop_duplicates()
    ...
    if '2021' in years:
        ...
        inputfiles = glob.glob(inputdir + '/2021*/ard2021S2.pkl')
        timeseries = pd.concat((iDF.set_index('parcelID') for iDF in map(pd.read_pickle, inputfiles)), sort=False, join='outer', axis=1).reset_index()


        inputfiles = glob.glob(inputdir + '/2021*/meta2021S2.pkl')
        metaseries = pd.concat(map(pd.read_pickle, inputfiles), sort=False, axis=0)
        metaseries2 = metaseries[chosenColumns].drop_duplicates()
    ...
    if '2022' in years:
        ...
        inputfiles = glob.glob(inputdir + '/2022*/ard2022S2.pkl')
        timeseries = pd.concat((iDF.set_index('parcelID') for iDF in map(pd.read_pickle, inputfiles)), sort=False, join='outer', axis=1).reset_index()


        inputfiles = glob.glob(inputdir + '/2022*/meta2022S2.pkl')
        metaseries = pd.concat(map(pd.read_pickle, inputfiles), sort=False, axis=0)
        metaseries2 = metaseries[chosenColumns].drop_duplicates()
    ...
    if '2023' in years:
        ...
        inputfiles = glob.glob(inputdir + '/2023*/ard2023S2.pkl')
        timeseries = pd.concat((iDF.set_index('parcelID') for iDF in map(pd.read_pickle, inputfiles)), sort=False, join='outer', axis=1).reset_index()


        inputfiles = glob.glob(inputdir + '/2023*/meta2023S2.pkl')
        metaseries = pd.concat(map(pd.read_pickle, inputfiles), sort=False, axis=0)
        metaseries2 = metaseries[chosenColumns].drop_duplicates()
...
def combineSingle2D(dates, inputdir, outputdir, norma):
    ...
    for date in dates:
        ...
        inputfile = os.path.join(inputdir + '/' + date + '/ard' + year + 'S2.pkl')
        ...
        # Read all meta files:
        inputfile = os.path.join(inputdir + '/' + date + '/meta' + year + 'S2.pkl')
        metaseries = pd.read_pickle(inputfile)
        metaseries2 = metaseries[['parcelID', 'target', 'count', 'startdate']].drop_duplicates()
